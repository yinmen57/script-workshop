...
| 现象 | 位置 | 说明 |
|------|------|------|
| 工具从未被执行 | `packages/governance/agent_workspace_service.py` | `entrypoint` 只在扫描时校验文件存在，从未被 `importlib` 加载 |
| 没有工具调用循环 | `packages/governance/chat_service.py` | 调 LLM 时不传 `tools`，单轮返回 |
...
新建 `packages/core/agent_runtime.py`，承担四件事：
