...
- 生成可靠性细节：[../../docs/business-script-material/09-generation-async-reliability.md](../../docs/business-script-material/09-generation-async-reliability.md)
- 方舟生图/生视频：[../../docs/business-script-material/08-ark-image-video-integration.md](../../docs/business-script-material/08-ark-image-video-integration.md)
...
1. **只通过封装投递**  
   业务代码禁止直接 `celery_app.send_task(...)` 散落；统一走 `framework.infra.jobs`（名称以落地为准）的 `enqueue_*` / `delay_*`。
...
| celery-worker-sync | `celery -A business.jobs.celery_app worker -Q sync -c N` | `sync` |
| celery-worker-gen | `celery -A business.jobs.celery_app worker -Q gen.submit,gen.finalize -c M` | 生成两队列 |
| celery-beat | `celery -A business.jobs.celery_app beat` | 不消费；只发周期任务 |

框架壳为 `framework.infra.celery_app`（仅 broker/序列化）；业务队列、Beat、任务模块由 `business.jobs.register` 挂载。
...
```text
framework/infra/celery_app.py           # 框架壳：broker / 序列化
framework/infra/jobs/enqueue.py         # 对外 enqueue API（框架）
business/jobs/
  register.py                          # 业务队列 / 路由 / Beat / imports
  celery_app.py                        # Worker/Beat 入口（壳 + register）
  sync_tasks.py                        # sync 队列任务
  gen_tasks.py                         # submit / finalize + Beat 任务
```
...
2. 在 `business/jobs` 增加 task，并在 `register.py` **登记队列名**。  
