    ...
    async def create_video_task(
        ...
        *,
        ...
        body: dict[str, Any] = {
            ...
            # 与 AI Key 配置的 model_name 对齐；上游不传则走租户默认
...
async def get_image_client(tenant_id: str) -> ShangwuClient:
    """仅赏舞；多 provider 请用 business.adapters.media_client.get_image_client。"""
    creds = await load_runtime_model(tenant_id, "image")
    provider = (creds.get("provider") or "shangwu").strip() or "shangwu"
    if provider != "shangwu":
        raise ValidationAppError(
            f"当前默认生图为 {provider}，请使用 business.adapters.media_client.get_image_client"
        )
...
async def get_video_client(tenant_id: str) -> ShangwuClient:
    """仅赏舞；多 provider 请用 business.adapters.media_client.get_video_client。"""
    creds = await load_runtime_model(tenant_id, "video")
    provider = (creds.get("provider") or "shangwu").strip() or "shangwu"
    if provider != "shangwu":
        raise ValidationAppError(
            f"当前默认生视频为 {provider}，请使用 business.adapters.media_client.get_video_client"
        )
    return ShangwuClient(
        kind="video",
        base_url=creds["base_url"],
